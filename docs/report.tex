...
\begin{itemize}
    ...
    \item \textbf{Giao diện web thời gian thực:} Dashboard 4 trang với WebSocket, đồng hồ gauge và điều khiển LED, NeoPixel (bật/tắt) cùng relay động.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{LED đơn (GPIO~48):} Chớp với 3 tốc độ tương ứng 3 mức nhiệt độ — 200~ms (nóng), 1000~ms (bình thường), 2000~ms (lạnh). Được bật/tắt qua nút toggle trên trang Device của dashboard; khi tắt, LED giữ mức LOW.
        \item \textbf{NeoPixel RGB (GPIO~45):} LED addressable WS2812B, thư viện \texttt{Adafruit\_NeoPixel}, độ sáng 50\%. Đổi màu theo 3 mức độ ẩm: Đỏ ($H < 40\%$), Xanh lá ($40\% \leq H \leq 60\%$), Xanh dương ($H > 60\%$). Được bật/tắt qua nút toggle trên dashboard; khi tắt, NeoPixel tắt hẳn (\texttt{pixels.clear()}) nhưng vẫn theo dõi màu hiện tại để khôi phục ngay khi bật lại.
...
\begin{itemize}
    ...
    \item \textbf{Không biến toàn cục sensor:} Dữ liệu truyền qua \texttt{xSensorQueue}; trạng thái thiết bị qua Semaphore. Ngoại lệ có kiểm soát: hai flag \texttt{led\_blinky\_enabled} và \texttt{neo\_blinky\_enabled} là biến toàn cục kiểu \texttt{bool} dùng để bật/tắt actuator từ WebSocket — không mang dữ liệu sensor.
...
\textbf{Điều Khiển ON/OFF qua Dashboard (Task~4):} Flag toàn cục \texttt{led\_blinky\_enabled} (mặc định \texttt{true}) nhận lệnh từ nút toggle trên trang Device. Khi \texttt{false}, task giữ LED ở mức LOW và \texttt{vTaskDelay(500~ms)} thay vì thực hiện chu kỳ chớp.
...
\textbf{Điều Khiển ON/OFF qua Dashboard (Task~4):} Flag \texttt{neo\_blinky\_enabled} nhận lệnh từ nút toggle trên trang Device. Task tách biệt hai pha: (1) luôn cập nhật \texttt{current\_color} từ semaphore kể cả khi disabled; (2) chỉ gọi \texttt{pixels.show()} khi màu hoặc trạng thái bật/tắt thay đổi. Khi bật lại, màu cũ được khôi phục ngay lập tức mà không cần chờ thay đổi semaphore mới.
...
\textbf{Hai thiết bị được điều khiển:} LED đơn (GPIO~48) và NeoPixel (GPIO~45) — mỗi thiết bị có một nút ON/OFF toggle riêng trên trang Device của dashboard.
...
WebSocket handler phân loại theo trường \texttt{page} trong JSON message:

\begin{table}[H]
\centering
\begin{tabular}{|l|l|p{6cm}|}
\hline
\textbf{page} & \textbf{Trường bổ sung} & \textbf{Hành động} \\
\hline
\texttt{actuator} & \texttt{device}, \texttt{status} & Bật/tắt LED (\texttt{led\_blinky\_enabled}) hoặc NeoPixel (\texttt{neo\_blinky\_enabled}) \\
\texttt{device}   & \texttt{value.gpio}, \texttt{value.status} & \texttt{digitalWrite} relay tùy chỉnh theo GPIO \\
\texttt{scan\_wifi} & — & Quét danh sách WiFi \\
\texttt{setting} & \texttt{value.ssid}, \texttt{value.password}, … & Lưu cấu hình, reboot \\
\hline
\end{tabular}
\caption{Phân loại WebSocket message theo trường \texttt{page}}
\end{table}

Khi người dùng nhấn nút toggle, JS gửi message \texttt{\{page:"actuator", device:"led"/"neo", status:"ON"/"OFF"\}}. Firmware cập nhật flag toàn cục tức thì; trạng thái được đồng bộ về client qua trường \texttt{led\_en}/\texttt{neo\_en} trong mỗi payload sensor broadcast.
...
\begin{enumerate}
    ...
    \item \textbf{Device:} Hiển thị trạng thái LCD (read-only); nút ON/OFF toggle cho \textbf{LED đơn} và \textbf{NeoPixel} — đây là 2 thiết bị và 2 nút điều khiển theo yêu cầu Task~4; nút thêm/xóa \textbf{relay động} theo GPIO tùy chỉnh.
...
\begin{figure}[H]
    ...
    \caption{Dashboard — Trang Device: nút ON/OFF cho LED \& NeoPixel và điều khiển relay động qua WebSocket}
...
% ============================================================
\section{ThingsBoard Rule Chain}
% ============================================================
...
\begin{enumerate}
    \item \textbf{Input} — Node \textit{Message Type Switch} tiếp nhận message và định tuyến theo loại (\texttt{POST\_TELEMETRY\_REQUEST}).
    \item \textbf{Phân loại dữ liệu} — Node \textit{Script (Transform)} chạy JavaScript kiểm tra giá trị \texttt{temperature} và \texttt{humidity}, gán nhãn trạng thái \texttt{NORMAL} / \texttt{WARNING} / \texttt{CRITICAL} vào metadata.
    \item \textbf{Lưu dữ liệu} — Node \textit{Save Timeseries} ghi telemetry vào database ThingsBoard để hiển thị lịch sử trên dashboard.
    \item \textbf{Kiểm tra logic cảnh báo} — Node \textit{Create \& Clear Alarm} tự động tạo hoặc xóa alarm dựa trên ngưỡng T/H, hiển thị cảnh báo trực tiếp trên CoreIOT dashboard.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth, keepaspectratio]{graphics/rulechains.png}
    \caption{Rule Chain trên CoreIOT: luồng Input $\rightarrow$ Phân loại $\rightarrow$ Lưu dữ liệu / Kiểm tra cảnh báo}
    \label{fig:rulechains}
\end{figure}

\subsection{Node Script JavaScript}

Node Script nhúng trực tiếp logic ngưỡng — đồng nhất với ngưỡng đã định nghĩa trong \texttt{include/global.h} của firmware:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth, keepaspectratio]{graphics/rulechains-js.png}
    \caption{Node Script JavaScript: kiểm tra ngưỡng T/H và gán nhãn trạng thái vào metadata}
    \label{fig:rulechains-js}
\end{figure}

\subsection{Gán Rule Chain Vào Device Profile}

Rule Chain được gán vào \textbf{Device Profile} của thiết bị ESP32-S3 thay vì Rule Chain mặc định, giúp cách ly logic xử lý theo từng loại thiết bị và dễ tái sử dụng khi thêm thiết bị mới:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth, keepaspectratio]{graphics/rulechains-profile-assign.png}
    \caption{Gán Rule Chain tùy chỉnh vào Device Profile trên CoreIOT}
    \label{fig:rulechains-profile}
\end{figure}

\subsection{Lợi Ích}

\begin{itemize}
    \item \textbf{Tách biệt edge và cloud:} Firmware chỉ publish JSON gốc; toàn bộ logic phân loại và cảnh báo nằm trên cloud, chỉnh sửa không cần flash lại thiết bị.
    \item \textbf{Alarm tự động:} ThingsBoard tạo/xóa alarm và có thể kích hoạt thông báo (email, Telegram) qua Rule Chain mà không cần code thêm phía ESP32-S3.
    \item \textbf{Lưu trữ có cấu trúc:} Dữ liệu được gán nhãn trạng thái trước khi lưu, dễ truy vấn và vẽ biểu đồ lịch sử theo mức cảnh báo.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Task 1:} Hoàn thành — LED GPIO~48 chớp đúng 3 tốc độ, semaphore edge detection hoạt động chính xác.
    \item \textbf{Task 2:} Hoàn thành — NeoPixel GPIO~45 hiển thị 3 màu theo độ ẩm, có toggle ON/OFF qua dashboard.
    \item \textbf{Task 3:} Hoàn thành — không biến toàn cục sensor; LCD cập nhật 500~ms/lần với 3 trạng thái qua mailbox queue.
    \item \textbf{Task 4:} Hoàn thành — dashboard 4 trang, gauge thời gian thực, điều khiển LED/NeoPixel và relay động; \textit{tồn đọng:} relay chưa lưu LittleFS sau reboot.
    \item \textbf{Task 5:} Đã triển khai — TinyML ANN 1928~bytes, 99.44\% accuracy trên dataset tổng hợp \cite{kaggle-iot-anomaly}, 6/6 test case biên đúng; chưa đánh giá trên dữ liệu thực.
    \item \textbf{Task 6:} Code hoàn chỉnh — MQTT kết nối, xác thực, publish JSON, reconnect; cần cấu hình access token thực qua trang Settings.
    \item \textbf{Rule Chain:} Triển khai thành công trên CoreIOT — phân loại dữ liệu, lưu timeseries và tạo alarm tự động theo ngưỡng T/H.
\end{itemize}
